...
class ProductDetailViewTest(TestCase):
    ...
    def setUp(self):
        ...
        user_model.objects.create_user('user1', 'user1@test.ru', 'password',
    ...
    def test_post_to_form_success(self):
        """Тестирование с отправкой Post запроса аутентифицированного пользователя
         с оценкой и коментом, проверка успешного сохранения в БД"""
        ...
        self.assertEqual(self.product1.feedbacks.first().author, 'goga gogoff')

    def test_post_authenticated_user_to_form_success(self):
        """Тестирование с отправкой Post запроса анонима
        с оценкой и коментом, проверка успешного сохранения в БД"""
        url_path = reverse('smokeshop:product_detail',
                           args=[self.category1.slug, self.product1.slug])
        response = self.client.post(url_path, data={
            'text': 'Some text', 'rating': 2
        })
        self.assertEqual(response.status_code, 302)
        self.assertEqual(self.product1.feedbacks.count(), 1)
        self.assertEqual(self.product1.feedbacks.first().text, 'Some text')
        self.assertEqual(self.product1.feedbacks.first().author, 'Анонимный пользователь')
